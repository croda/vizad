...
class Camec(Problem):
    ...
    def __init__(self, initial, camec, goal):
        self.initial = initial
        self.camec = camec
        self.goal = goal
      ...
      if(pravec == 'L'):
          for i in range(1,c+1):
            for j in range(0,i+1):
              if(self.validateSuccLeft(j,i,ML,KL,MD,KD)):
                successors[str(i-j)+'M_'+str(j)+'K'] = (ML-(i-j),KL-j,'D',MD+(i-j),KD+j)         
      elif(pravec == 'D'):
          for i in range(1,c+1):
            for j in range(0,i+1):
              if(self.validateSuccRight(j,i,ML,KL,MD,KD)):
                successors[str(i-j)+'M_'+str(j)+'K'] = (ML+(i-j),KL+j,'L',MD-(i-j),KD-j)
 
      return successors
    
    def validateSuccLeft(self,j,i,ML,KL,MD,KD):
        return ((ML-(i-j))>=0 and (KL-(j))>=0 and ((ML-(i-j))>=(KL-(j)) or (ML-(i-j))==0) and ( (MD+(i-j))>=(KD+j) or (MD+(i-j))==0 ))
    
    def validateSuccRight(self,j,i,ML,KL,MD,KD):
        return (MD-(i-j))>=0 and (KD-(j))>=0 and ((ML+(i-j))>=(KL+(j)) or (ML-(i-j))==0) and ((MD-(i-j))>=(KD-(j)) or ((MD-(i-j))==0))
...
CamecInstance = Camec((int(N), int(N), 'L', 0, 0),int(K), int(N))

#answer = greedy_best_first_graph_search(CamecInstance)
answer = astar_search(CamecInstance)
print (answer.solve())
